...
int mychrdev_ioctl(struct file * file, unsigned int cmd, unsigned long argp)
{
	printk("entry mychrdev_ioctl:%p\n", file);
        ...
        switch(cmd) 
	{
			...
			printk("window.head:%d, window.tail:%d\n", window.head, window.tail);
